...
static double keep_for(int V) {
    ...
    if (V <= 40000)  return 0.1475;// case 4: PROBE 85.25% (85 pass local 0.8638, 85.5 WA local 0.8609 -> 0.8625 = midpoint coin flip)
    if (V <= 100000) return 0.0925;// case 5: 90.75% confirmed (90.80+stack WA'd v48, 91 WA'd twice -> CLOSED)
    if (V <= 400000) return 0.0275;// case 6: 97.25% w/ VSA CONFIRMED v53
    return 0.0295;                 // case 7: PROBE 97.05% w/ VSA (97.2 WA v53 -> proxy over-predicts >=0.0034; 97.05 local margin +0.0065)
}
...
static int refine_for(int V) { return ((V > 7000 && V <= 40000) || V > 100000) ? 1 : 0; }  // case3+case4+case6+case7 (case5 opt no help; case6 +0.0053 / case7 +0.001 local w/ VSA)
...
static int ndecim_for(int V) { return (V > 7000) ? 1 : 0; }  // all cases 3-7 (case2's 52-vert output: untested, off)
