...
static double keep_for(int V) {
    ...
    if (V <= 30000)  return 0.30;  // case 3: PROBE 70% (69.5 confirmed v47; local 0.8979 vs 0.8974 ref — coin flip)
    if (V <= 40000)  return 0.1575;// case 4: PROBE 84.25% w/ stack (84.10 confirmed v47; 84.25 WA'd only pre-vis/projw)
    if (V <= 100000) return 0.092; // case 5: PROBE 90.80% + vis+projw newly on (90.75 confirmed; 91 WA'd twice pre-vis)
...
static int projw_for(int V) { return (V > 30000 && V <= 100000) ? 1 : 0; }  // case4 (v46-confirmed) + case5 (+0.0009 local)
...
int main(int argc, char** argv) {
    ...
    {   // view-aware: free the hidden (never-rendered) geometry so the budget goes to visible faces
        ...
        bool vis = (VV > 7000 && VV <= 100000);                                 // case3 + case4 (+0.0032) + case5 (+0.0003 local)
