...
static double keep_for(int V) {
    if (V <= 7000)   return 0.0075;// case 2: confirmed (pays 99.268)
    if (V <= 30000)  return 0.305; // case 3: 69.5 confirmed
    if (V <= 40000)  return 0.150; // case 4: 85 confirmed (razor edge, hold)
    if (V <= 100000) return 0.09;  // case 5: PUSH 91% w/ ST-refine (0.8561 local = +0.0038 over passing; old 91-WAs were refine-less)
    if (V <= 400000) return 0.025625;// case 6: 97.4375 CONFIRMED (v62)
    return 0.029;                  // case 7: 97.10 confirmed; wall in (97.10,97.15)
}
...
static int refine_for(int V) { return (V > 7000 && V <= 400000) ? 1 : 0; }  // cases 3-6; case5 added: ST 16s-box refine converges (0.8561@91%, +0.0038 over passing level). case7 stays off (v55 TLE). SINGLE-THREAD ONLY: judge bills cumulative CPU across threads (v60/v63 lesson).
