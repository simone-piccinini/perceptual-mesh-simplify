...
// This build PROBES cases 2,4,5 lower (keep 0.15/0.25, were 0.30 confirmed) to find their
// SSIM floor; case 3 + large stay confirmed-safe. If all pass -> ~81.5. best so far = 77.34
// (submissions/v9-percase-keep); best-counts protects it on any WA.
...
static double keep_for(int V) {
    if (V <= 7000)  return 0.15;   // case 2 (tiny): PROBE 85% (0.30 confirmed; small meshes hold SSIM very high)
    if (V <= 30000) return 0.36;   // case 3: fragile, 0.30 FAILED -> keep proven 0.36
    return 0.25;                   // cases 4,5: PROBE 75% (0.30 confirmed)
}
