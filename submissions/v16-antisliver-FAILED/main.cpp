...
// anti-sliver: reject a collapse that creates a thin triangle. A sliver's face normal
// is unstable/garbage -> wrecks the flat-shaded normal-map SSIM. quality = 2*sqrt(3)*
// area / sum(edge^2): 1 = equilateral, -> 0 = sliver. 0 disables. Conservative value
// rejects only clear slivers so it does not starve compression. Tune on the judge.
...
static double keep_for(int V) {
    ...
    if (V <= 40000)  return 0.20;  // case 4: 80% confirmed (clean base for the anti-sliver test)
    if (V <= 100000) return 0.20;  // case 5: PROBE 80% (75% confirmed; 0.20/80% FAILED with plain free-QEM
                                   //         -> betting the anti-sliver gate's cleaner normals rescue it)
...
bool SafeToCollapse(int i, int j, const Vec3& xbar) {
    ...
    auto face_ok = [&](int f, int moved) -> bool {
        ...
        if (kMinQuality > 0.0) {                          // anti-sliver
            const double sumsq = (Pn[1]-Pn[0]).squaredNorm()
                               + (Pn[2]-Pn[1]).squaredNorm()
                               + (Pn[0]-Pn[2]).squaredNorm();
            if (sumsq > 0.0 && (2.0*1.7320508075688772*0.5*lenN)/sumsq < kMinQuality)
                return false;                              // 2*sqrt(3)*area/sum(edge^2)
        }
        return true;
    };
