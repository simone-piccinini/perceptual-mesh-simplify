...
// This build PROBES cases 2,4,5 lower again (keep 0.10/0.20; 0.15/0.25 confirmed) to find
// their SSIM floor; case 3 + large stay confirmed-safe. If all pass -> ~84. best so far =
// 81.50 (submissions/v10-keep-push); best-counts protects it on any WA.
...
static double keep_for(int V) {
    if (V <= 7000)  return 0.10;   // case 2 (tiny): PROBE 90% (0.15 confirmed PASS)
    if (V <= 30000) return 0.36;   // case 3: fragile, 0.30 FAILED -> keep proven 0.36
    return 0.20;                   // cases 4,5: PROBE 80% (0.25 confirmed PASS)
}
