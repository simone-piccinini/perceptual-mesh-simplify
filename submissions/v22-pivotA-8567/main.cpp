...
static double keep_for(int V) {
    ...
    if (V <= 30000)  return 0.35;  // case 3: PROBE 65% with Pivot-A steering (free-QEM capped 64%)
    if (V <= 40000)  return 0.18;  // case 4: 82% confirmed (free-QEM; 83% WA'd with AND without Pivot-A)
    if (V <= 100000) return 0.20;  // case 5: PROBE 80% with Pivot-A steering (free-QEM capped 79%, 80% WA'd)
...
static double lambda_for(int V) {
    ...
    if (V > 40000 && V <= 100000) return 12.0;   // case 5 (Pivot-A broke 79->80 on the judge)
    return 0.0;                                  // cases 2,4,6,7 (case4 free-QEM: 83% WA'd, no steering gain)
}
