...
static double keep_for(int V) {
    ...
    if (V <= 100000) return 0.095; // case 5: VSA-lite -> 90.5% (JUDGE-CONFIRMED pass)
...
static int ndecim_for(int V) { return ((V > 7000 && V <= 30000) || (V > 40000 && V <= 100000)) ? 1 : 0; }  // case3 + case5
...
static double incident_ndist(int i, int j, const Vec3& xbar) {
    ...
    auto acc = [&](int moved, int other){
        for (int f : vfaces[moved]) {
            ...
            nd += 0.5*ln*(1.0 - (co/lo).dot(cn/ln));   // area_new * (1 - cos angle)
        }
    };
    acc(i,j); acc(j,i);
    return nd;
...
int main(int argc, char** argv) {
    ...
    {   // view-aware: free the hidden (never-rendered) geometry so the budget goes to visible faces
        ...
        if (VV > 7000 && VV <= 30000) { compute_visibility(); if (g_lambda <= 0.0) seed_heap(); }  // case3
    }
